...
namespace SimsProject.Domain.Model
{
    public class AccommodationReview : ISerializable
    {
        ...
        public AccommodationReview(int cleanlinessGrade, int ownerGrade, string comment, User owner, User guest, AccommodationReservation reservation, List<Image> images)
        {
        ...
        public string[] ToCsv()
        {
            string[] csvValues = { Id.ToString(), CleanlinessGrade.ToString(), OwnerGrade.ToString(), Comment, Owner.Id.ToString(), Guest.Id.ToString(), Reservation.Id.ToString() };
            return csvValues;
        }
